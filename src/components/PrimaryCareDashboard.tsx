...
export default function PrimaryCareDashboard() {
  ...
  const isFilterActive = useMemo(() => {
    return directorySearch.trim().length > 0 || 
          ...
          {!isFilterActive && !hasClickedBrowseAll ? (
            <div className="bg-slate-950 border border-slate-900 rounded-xl p-12 text-center space-y-4">
              ...
              <div className="max-w-md mx-auto space-y-2">
                ...
                <p className="text-xs text-slate-400">
                  Enter a clinic name, physician name, city, or PCN, or use the filters above to locate accepting family physicians and clinics.
                </p>
                <div className="pt-2">
                  <button
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {paginatedProviders.map(prov => { return (
                  <div
                    key={prov.id}
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between border-t border-slate-900 pt-4 mt-6">
                  ...
                  <span className="text-xs text-slate-400 font-medium">
                    Page <strong className="text-white font-black">{currentPage}</strong> of <strong className="text-white font-black">{totalPages}</strong>
                  </span>
                  <button
