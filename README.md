...
- **2026-07-22 — Urgent Care tab:** primary nav is ER Waits | Urgent Care | Diagnostics & Labs; UC sites (5 feed-backed Calgary-area centres) use the shared wait dashboard with `scope`. Pages production branch is `main` (matches git default). Cloudflare Pages project production branch was corrected from the historical `test` pin to `main`.
...
npm run deploy:pages    # frontend → Cloudflare Pages (branch main = production)
