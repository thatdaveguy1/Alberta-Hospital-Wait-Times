...
### Lesson: agent-browser CLI requires --headed and --session on every command
- **Mistake:** Relied on `BROWSER_HEADED` / `AGENT_BROWSER_SESSION` env vars only; used `ab reload` (not a stable CLI op) and `location.reload()` inside `eval`, which detached CDP and broke fill/snapshot.
- **Solution:** `scripts/headed-audit-verify.sh` wraps `agent-browser "$@" --headed --session "$AB_SESSION"`, runs `close --all` before proof, seeds location via `localStorage` then **`open $BASE_URL` again** (no in-eval reload), uses `scrollintoview` / `#contributions` for the on-page contributions block.
- **Prevention:** For headed flows use a dedicated session name; never mix OMP `browser` tool and `agent-browser` on the same URL without closing one side first.
...
### Lesson: Distinguish true zeros from missing data in dashboards
- **Mistake:** The System Flow dashboard rendered `0%` occupancy, `0h` P90 bed wait, and `0%` ALC for facilities that simply had no HQA FOCUS estimate. Users interpreted the zeros as real values rather than missing data.
- **Solution:** Added a `hasFlowData` helper in `SystemFlowDashboard.tsx` that treats `hospitalOccupancy === 0 && p90BedWait === 0 && alcRate === 0` as missing. Cells now render `—` for missing metrics, real-data facilities are sorted to the top of the grid, and a note explains that flow metrics come from HQA FOCUS and aren't available for all facility types. Provincial averages for ALC, LWBS, and P90 wait now exclude zero values.
- **Prevention:** When a data source uses `0` as a default/placeholder for missing values, the UI must explicitly distinguish missing from measured zero. Prefer explicit null/undefined in the data model, or at least render a clear placeholder and exclude missing values from averages.

### Lesson: Weekly ED LOS reports only cover major Edmonton/Calgary hospitals
...
### Lesson: Missing _dataMetadata makes manual dashboards show Unknown/—
- **Mistake:** Six data files (data-cancer.json, data-continuing-care.json, data-mental-health.json, data-public-health.json, data-patient-experience.json, data-workforce.json) had no `_dataMetadata` block. The new standardized `DashboardHeader` component relied on `_dataMetadata`, so these tabs showed `MANUAL UPDATE`, `Last Update: Unknown`, and `Data Timestamp: —`.
- **Solution:** Injected real `_dataMetadata` for every array in those files, mapping each to its actual source, sourceVintage, and updateType. Fixed two dashboard header `arrayKey` mismatches (`PatientExperienceDashboard` used `PATIENT_SATISFACTION_STATS` which does not exist; `RegionalInequityDashboard` used `PROVINCIAL_BENCHMARKS` which does not exist). Updated every scraper/fetcher to preserve existing `_dataMetadata` via `mergeDataMetadata` and stamp the arrays it refreshes via `buildMetadataEntry`.
- **Prevention:** Every `data-*.json` file must have `_dataMetadata` covering every array. Every pipeline that writes a data file must merge metadata, not overwrite it. Dashboard headers must use an `arrayKey` that actually exists in the data file.

### Lesson: Stale data when the daily-sync launchd job is not loaded
- **Mistake:** Dashboard headers showed `AUTO-UPDATED (2D)` with old `Last Update` timestamps. The daily pipeline `com.alberta-hospital-pipeline-daily` was not running, so data files had not been refreshed since 2026-07-05 even though today was 2026-07-07.
- **Solution:** Removed the misleading relative-time suffix from the dashboard badge so it now simply reads `AUTO-UPDATED` or `MANUAL UPDATE`. The `Last Update` line continues to show the scraper run time and the `Data Timestamp` line shows the source vintage. Kicked off the daily sync (`launchctl kickstart`) and confirmed it is refreshing all domains; the job is loaded and scheduled for 6:00 AM daily.
- **Prevention:** After any maintenance that unloads launchd jobs, reload/enable the daily pipeline. Monitor `data-sync-status.json` and dashboard `Last Update` timestamps. Do not rely on a relative-time badge to communicate data vintage — it only reflects scraper run time, which can be much newer than the underlying source data.

### Lesson: Charts must trim leading zero/placeholder years instead of stretching to the full dataset range
...
### Lesson: Cloudflare Pages production branch is independent of git default
- **Mistake:** Deployed a frontend-only Urgent Care tab split with `wrangler pages deploy dist --project-name alberta-hospital-wait-times` (no `--branch`). Wrangler treated the upload as a **Preview** for branch `main`. Production (`alberta-hospital-wait-times.pages.dev`) stayed on an older build because the project's Pages **production branch** was historically pinned to **`test`**, not git `main`. Users saw no Urgent Care tab; only `main.alberta-hospital-wait-times.pages.dev` and the deploy preview had the new nav.
- **Solution (temporary):** Redeploy the same `dist/` with `--branch test` so Cloudflare marks it **Production** and updates the live hostname.
- **Solution (permanent):** PATCH the Pages project `production_branch` to `main` via the Cloudflare API (`/accounts/{account_id}/pages/projects/{project}`), update `npm run deploy:pages` to `--branch main`, and redeploy. Confirm with `npx wrangler pages deployment list --project-name alberta-hospital-wait-times` that the newest row is Environment=`Production`, Branch=`main`. Hard-refresh or check `index.html` for the new hashed asset — edge POPs can briefly serve mixed old/new HTML.
- **Prevention:** Keep Pages production branch aligned with git default (`main`). Always pass an explicit `--branch` in `deploy:pages`. After deploy, verify the bare `*.pages.dev` production hostname (not only the deploy preview). Use `npx wrangler` from the repo root (wrangler is not on PATH there). Pass `--commit-dirty=true` when local data JSON refresh files are intentionally uncommitted.
